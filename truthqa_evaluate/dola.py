...
STOPPING_CRITERIA_INPUTS_DOCSTRING = r"""
    Args:
        ...
        scores (`torch.FloatTensor` of shape `(batch_size, config.vocab_size)`):
            ...
            each vocabulary token before SoftMax or scores for each vocabulary token after SoftMax.
        kwargs:
...
class StoppingCriteria(ABC):
    ...
    that can be applied during generation.
    """
...
class LLamaQaStoppingCriteria(StoppingCriteria):
    """
       This class can be used to stop generation whenever
    the model generates '\nQ:' tokens. It means that the model
    has finished generating the answer and start generating a new question.
    """
...
class DoLa:
    def __init__(self, model_name, device, num_gpus, max_gpu_memory=27):
        ...
        print("\n=>=> You are loading model and tokenizer from {}".format(model_name))
    ...
    def generate(self, input_text, max_new_tokens=256, top_p=0.95, top_k=0, temperature=0.8, mature_layer=None,
                 premature_layer=None, candidate_premature_layers=[], mode='baseline', verbose=True,
                 remove_stop_words=False, relative_top=0.1, **kwargs):
        with torch.no_grad():
    ...
    def lm_score(self, input_text1, input_text2, pmi=False, max_new_tokens=256,
                 top_p=0.95, top_k=0, temperature=0.8,
                 mature_layer=None, premature_layer=None,
